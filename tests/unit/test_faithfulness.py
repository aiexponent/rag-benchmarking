...
def test_faithfulness_full_support():
    with patch("rag_benchmarking.app.eval.faithfulness.LLMClient", return_value=_mock_llm(MOCK_FULL_SUPPORT)):
...
def test_faithfulness_partial_support():
    with patch("rag_benchmarking.app.eval.faithfulness.LLMClient", return_value=_mock_llm(MOCK_PARTIAL_SUPPORT)):
        result = compute_faithfulness("Some answer.", ["Some context."])
    assert result["score"] == pytest.approx(0.5)


def test_faithfulness_no_claims_returns_neutral():
    with patch("rag_benchmarking.app.eval.faithfulness.LLMClient", return_value=_mock_llm(MOCK_NO_CLAIMS)):
...
def test_faithfulness_malformed_response_returns_zero():
    with patch("rag_benchmarking.app.eval.faithfulness.LLMClient", return_value=_mock_llm(MOCK_MALFORMED)):
        result = compute_faithfulness("Some answer.", ["Some context."])
...
def test_faithfulness_strips_markdown_codeblock():
    with patch("rag_benchmarking.app.eval.faithfulness.LLMClient", return_value=_mock_llm(MOCK_MARKDOWN_WRAPPED)):
